...
suite('TerminalSandboxService - network domains', () => {
	...
	test('should prefix wrapped command with ELECTRON_RUN_AS_NODE=1 when no remote env is available', async () => {
	...
	test('should place sandbox temp dir under the local data folder when no remote env is available', async () => {
		remoteAgentService.remoteEnvironment = null;
		const sandboxService = store.add(instantiationService.createInstance(TerminalSandboxService));
		await sandboxService.getSandboxConfigPath();
